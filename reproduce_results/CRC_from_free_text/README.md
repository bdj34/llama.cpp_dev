...
This directory contains the pipeline for identifying patients diagnosed with Colorectal Cancer (CRC) and extracting the CRC diagnosis date from any/all clinical notes. This process is designed to supplement other sources of data which are known to be incomplete (e.g., VA cancer registry).
...
1.  **Initial run:** Two models (Llama-70B and Mistral-24B) initially screen all patients using regex-derived snippets of their clinical notes.
2.  **Targeted re-run:** Patients flagged as "Positive" by either model in the first sweep are re-processed with expanded context. 
    * **Note:** During this re-run, Mistral receives less context than Llama and Gemma due to its smaller context window limit.
3.  **Consensus:** A final diagnosis is confirmed only if 4 out of 5 (2 initial + 3 re-runs) model answers agree on a CRC diagnosis.
...
4.  **Identify "Either Positive" Cohort (`step4_aggregate_crc_free_text...R`):**
    ...
    * Identifies any patient where at least one model said "Yes".

5.  **Prepare Expanded Inputs:**
    ...
    * **For Mistral (`step5A_...py`):** Generates prompts with moderate context (15 lines before/10 lines after) to accommodate its smaller context window.

6.  **Re-Run Inference (Round 2):**
    * **Mistral-Small-24B** (`step6A_...sh`) runs on the "Either Positive" cohort with slightly expanded context.
...
7.  **Final Consensus & Date Extraction (`step7_aggregate_crc_free_text...R`):** * **Consensus Rule:** A patient is confirmed as CRC positive only if 4 or 5 of the 5 LLM answers are "Yes".
...
| **LLM run on added context only only** | [Gemma-3-27B (Q6_K_L)](https://huggingface.co/bartowski/google_gemma-3-27b-it-GGUF/blob/main/google_gemma-3-27b-it-Q6_K_L.gguf) |
